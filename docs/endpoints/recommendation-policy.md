...
This endpoint returns **language-agnostic structured data** — all recommendation content is represented as stable keys (`id`, `activity_keys`, `category`). Wizdam Sikola is responsible for translating these keys into user-facing text via its own i18n system.

Each recommendation item uses its `id` (e.g. `GOV-01`) as the primary translation key. Activity keys (e.g. `modernize_research_labs`) map to localized text in Wizdam Sikola's translation files.
...
| `research_landscape` | object | No | `{}` | Aggregated stats from Wizdam Sikola DB — enables data-driven recommendations |
...
| `integrate_wizdam_sikola_tracking` | INST-03 — Use Wizdam Sikola |
| `sync_orcid_scopus_sinta` | RES-01 — Profile synchronization |
| `wizdam_sikola_impact_monitoring` | RES-01 — Use impact dashboard |
...
Since the API returns keys, Wizdam Sikola must maintain a translation file. Example structure:
...
Aggregate the `research_landscape` from Wizdam Sikola's analytics tables before calling the API:
...
Each stakeholder type maps to a different Wizdam Sikola UI section:
